...
from dobotBindings.message import Message
...
class Dobot(threading.Thread):
	...
	def _set_end_effector_gripper(self, openGripper):
		msg = Message()
		msg.id = 63
		msg.ctrl = 0x03
		msg.params = bytearray([])
		msg.params.extend(bytearray([0x01]))
		if openGripper is True:
			msg.params.extend(bytearray([0x01]))
		else:
			msg.params.extend(bytearray([0x00]))
	...
	def openGripper(self):
		return self._set_end_effector_gripper(True)

	def closeGripper(self):
		return self._set_end_effector_gripper(False)

	def go_angle(self, j1, j2, j3, j4=0):
		self._set_ptp_cmd(j1, j2, j3, j4, mode=MODE_PTP_MOVJ_ANGLE)

	def go(self, x, y, z, r=0.):
		self._set_ptp_cmd(x, y, z, r, mode=MODE_PTP_MOVJ_XYZ)

	def go_angle_inc(self, angle_j1, angle_j2, angle_j3, angle_j4=0):
		self._set_ptp_cmd(angle_j1, angle_j2, angle_j3, angle_j4, mode=MODE_PTP_MOVJ_INC)
